...
|:--------:|:-----------:|:--------:|:--------:|
| **pg_replication_postgresql_version** | This variable dictates what configuration will be used for the version of your Postgresql server installed | Yes | 13 |
...
| **pg_replication_type** | Used to determine the type of replication that will be used. Options are logical and log_shipping_with_streaming. | Yes | log_shipping_with_streaming |
...
| **pg_replication_port** | The port used by the Postgresql servers| yes | 5432 |
...
| **pg_replication_postgresql_conf_file**| The name of the configuration file postgresql uses| Yes | postgresql.conf|
...
Below is a list of Postgresql parameters that can be changed with variables for the role.
| Variable | Description | Required | Defaults |
|:--------:|:-----------:|:--------:|:--------:|
|**pg_replication_param_archive_mode**|Enables the ability to archive wal files.|no| "off"|
|**pg_replication_param_listen_address**|What network address is Postgresql listenting on.|no| '*'|
|**pg_replication_param_max_connections**|Maximum connections that can be used for replication.|no| 100|
|**pg_replication_param_shared_buffers**|Memory size for shared buffers.|no| 128MB|
|**pg_replication_param_work_mem**|Memory size for working memory.|no| 4MB|
|**pg_replication_param_maintenance_work_mem**|Memory size for maintenance working memory.|no| 64MB|
|**pg_replication_param_wal_keep_segments**|Number of wall keep segements for Postgresql 11 and older. For newer Postgresql this is the wal_keep_size, which is normally 64MB.|no| 8|
|**pg_replication_param_archive_timeout**|Timeout used for archives.|no| 60|
|**pg_replication_param_max_wal_size**|Maximum size of a wal.|no| 1GB|
|**pg_replication_param_min_wal_size**|Minimum size of a wal.|no| 128MB|
|**pg_replication_param_max_replication_slots**|Maximum replication slots.|no| 60|
|**pg_replication_param_max_concurrent_connections**|Maximum concurrent connections.|no|10|
|**pg_replication_param_max_logical_replication_workers**|Maximim replication workers.|no| 60|
|**pg_replication_param_max_worker_processes**|Maximum number of worker processes.|no| 60|
...
pg_slot_names:
  slot1:
    slot_server_name: server1.example.com
    slot_slot_name: firststandby
  slot2:
    slot_server_name: server2.example.com
    slot_slot_name: secondstandby
